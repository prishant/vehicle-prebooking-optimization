...
%[randi(Np,D,D/2) mod(randi(Np*1000,D,D/2),5)];
P = repmat(lb,Np,1) + repmat((ub-lb),Np,1).*randi(Np,D);   % Generation of the initial population
TF=1;
P = [mod(P(:,1:7),48) mod(P(:,8:14),5)+1];

for p = 1:Np
    f(p) = round(prob(P(p,:)));            % Evaluating the fitness function of the initial population
end
...
for t = 1:T
    for i = 1:Np
        Xmean = round(mean(P));            % Determining mean of the population
        Xmean
        ...
        Xnew = P(i,:) + round(rand(1,D).*(Xbest - TF*Xmean));  % Generating the new solution
        Xnew
        Xnew = [mod(Xnew(:,1:7),48) mod(Xnew(:,8:14),5)+1];
        ...
        fnew = round(prob(Xnew));          % Evaluating the fitness of the newly generated solution
        ...
        if f(i)< f(p)    % Select the appropriate equation to be used in Learner phase
            Xnew = P(i,:) + round(rand(1, D).*(P(i,:) - P(p,:)));  % Generating the new solution
            Xnew = [mod(Xnew(:,1:7),48) mod(Xnew(:,8:14),5)+1];
        else
            Xnew = P(i,:) - round(rand(1, D).*(P(i,:) - P(p,:)));  % Generating the new solution
            Xnew = [mod(Xnew(:,1:7),48) mod(Xnew(:,8:14),5)+1];
